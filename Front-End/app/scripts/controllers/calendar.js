    ...
    .controller('calendarController', ['$scope', '$http', '$location', '$window', '$cookies', '$timeout', '$interval', 'moment', 'databaseService', 'sharedService', 'authenticationService', 'emailService', 'globalizationService', 'commonService',
        function($scope, $http, $location, $window, $cookies, $timeout, $interval, moment, databaseService, sharedService, authenticationService, emailService, globalizationService, commonService) {
            ...
            this.initCalendarDays = function() {
                ...
                this.date = this.date.year($scope.year).isoWeek($scope.week).isoWeekday(1);
                this.dateDisplay.set('year', $scope.year);
                this.dateDisplay.set('week', $scope.week);
            ...
            this.setWeek = function(indexOfWeeksArray) {
                ...
                var newWeekDate = moment().year(weekData.year).isoWeek(weekData.week).isoWeekday(1);
                var currentWeekDate = moment().year($scope.year).isoWeek($scope.week).isoWeekday(1);
                this.date = newWeekDate;
                ...
                if (newWeekDate.month() !== currentWeekDate.month()) {
                    ...
                    commonService.initMonthData(this, newWeekDate.month(), newWeekDate.year());
                    this.initCalendarDays();
                ...
                this.dateDisplay.set('year', $scope.year);
                this.dateDisplay.set('week', $scope.week);
                this.dateDisplay.year(weekData.year).isoWeek(weekData.week).isoWeekday(1);
            };
            ...
            $scope.handleErrorDB = function(status, data) {
                ...
                if (data !== undefined && data !== null) {
                    ...
                    $scope.error += data.msg + ':' + data.error + ' | ';
                } else {
