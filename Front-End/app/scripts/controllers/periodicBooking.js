...
angular
    .module('calendarApp')
    .controller('periodicBookingController', ['$scope', '$cookies', '$window', '$timeout', '$location', 'moment', 'sharedService', 'databaseService', 'globalizationService', 'emailService', 'authenticationService', 'commonService',
        function PeriodicBookingController($scope, $cookies, $window, $timeout, $location, moment, sharedService, databaseService, globalizationService, emailService, authenticationService, commonService) {
            moment.locale('fr');
            ...
            this.dateEnd = moment();
            this.dateDisplayStart = moment();
            this.dateDisplayEnd = moment();
            ...
            this.dateDisplayStart.locale($window.navigator.userLanguage || $window.navigator.language);
            this.dateDisplayEnd.locale($window.navigator.userLanguage || $window.navigator.language);
            ...
            this.initWeek = function() {
                commonService.initMonthData(this, this.todayMonth, this.todayYear, true, true);
                commonService.initMonthData(this, this.todayMonth, this.todayYear, true, false);
            };

            this.setMonthData = function(month, mode, isStartingDate) {
                var newDate;
                if (isStartingDate) {
                    if (mode === 'add') {
                        newDate = this.dateStart.add(1, 'M');
                    } else if (mode === 'sub') {
                        newDate = this.dateStart.subtract(1, 'M');
                    }
                } else {
                    if (mode === 'add') {
                        newDate = this.dateEnd.add(1, 'M');
                    } else if (mode === 'sub') {
                        newDate = this.dateEnd.subtract(1, 'M');
                    }
                }


                commonService.initMonthData(this, newDate.month(), newDate.year(), true, isStartingDate);
            };
            ...
            this.selectStartingDay = function(monthDay, week, year) {
                if (this.periodicBookingStartingDay !== undefined &&
                    ...
                    if (monthDay.month !== this.selectedStartMonth) {
                        commonService.initMonthData(this, monthDay.month, year, true, true);
                    }
                }
                ...
                if (this.selectedEndDay !== undefined &&
                    ...
                    this.periodicBookingWeeksDuration = endingDay.diff(startingDay, 'weeks');
            ...
            this.selectEndingDay = function(monthDay, week, year) {
                if (this.selectedEndDay !== undefined &&
                    ...
                    if (monthDay.month !== this.selectedEndMonth) {
                        commonService.initMonthData(this, monthDay.month, year, true, false);
                    }
                }
